...
class ClickhouseToSQLitePipeline:
    def __init__(
        self,
        delte_table_path="tmp/airports_raw_delta_1",
        sqllite_table_path="tmp/temp_airports.db",
    ):
        """
    ...
    def extract_load(self):
        ...
        df = self.clickhouse_client.query_df("SELECT * FROM airports")
        print(df.head(1).to_dict(orient="records"))
        self.logger.info("Data extraction complete. Writing to Delta Lake.")

        # partition_by=["Country"],
        self.write_deltalake(self.delta_table_path, df, mode="overwrite")
    ...
    def data_quality_check(self, sqllite_path="temp_airports.db"):
        """
        ...
        conn.execute(f"ATTACH '{sqllite_path}' (TYPE SQLITE); USE temp_airports;")
        ...
        row_count_sqlite = conn.execute("SELECT COUNT(1) FROM airports").fetchone()[0]

        self.logger.info(f"Row count in SQLite database: {row_count_sqlite}")
        conn.close()
        return row_count_sqlite

    def write_deltalake(self, path, df, partition_by=None, mode=None):
        """
